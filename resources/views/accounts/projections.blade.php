                ...
                <div class="page-body">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3>Projections for year {{ $set_year }}</h3>

                        </div>
                        <div class="card-header">
                            <form class="form-inline" action="{{ url('budget/projections') }}" method="get">@csrf
                                <div class="form-group">
                                    <a class="btn btn-info btn-sm" href="{{ url('budget/projections/create')}}">New
                                        Projection</a>
                                </div>
                                <div class="form-group col-xs-offset-1">
                                    ...
                                    <select name="year" id="year">
                                        @foreach($years as $t_year)
                                            <option value="{{ $t_year }}"
                                                    @if($set_year == $t_year) selected="selected" @endif>{{ $t_year }}</option>
                                        @endforeach
                                    </select>
                                </div>
