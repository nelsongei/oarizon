   ...
   <div align="center"><strong>Sales Report as from:  {{$from}} To:  {{$to}}</strong></div><br>
    ...
    <table class="table table-bordered" border='1' cellspacing='0' cellpadding='0'>
      ...
      <tr>
        ...
        <th align="center"><strong>VAT Amount </strong></th>
      ...
      @if($clientobj->id == $sales[client_id])
      @if($sales[status] != "REJECTED" && $sales[status] != "cancelled" )
      
      <?php
      $total = $total + ($sales['totalamount'] + $sales['tax']);
      ...
      <tr>
       ...
       <td td width='20'>{{$i}}</td>
        ...
        <td> {{ $sales['date'] }}</td>
        <td> {{ $sales['client'] }}</td>
        <td> {{ asMoney($sales['tax']) }}</td>
        <td align = "right"> {{ asMoney($sales['totalamount'] + $sales['tax'])}}</td>
